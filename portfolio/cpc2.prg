...
do while .not. eof()
   vweight = c->weight - weight
   do case
   case vweight > 0
      replace weight with vweight
      replace cluster with 1
      replace label with trim(substr(descript,1,77))
   case vweight < 0
      replace weight with -vweight
      replace cluster with 2
      replace label with trim(substr(descript,1,77))
   case vweight = 0
      replace weight with 0.15
      replace cluster with 1000
